...
public class SpinCADBlock extends SpinFXBlock {
	...
	/**
	 * Called after deserialization. Fixes up fields added in newer versions
	 * that default to 0.0 instead of their intended initial value.
	 */
	protected Object readResolve() {
		// Use reflection to find private double fields with value 0.0
		// whose declaring class also has a setter generated by @sliderLabel DBLEVEL.
		// Those setters use Math.pow(10, x/20), so a raw 0.0 means the field
		// was absent in the serialized stream — fix it to 1.0 (unity / 0 dB).
		for (java.lang.reflect.Field f : getClass().getDeclaredFields()) {
			if (f.getType() == double.class) {
				try {
					f.setAccessible(true);
					if (f.getDouble(this) == 0.0) {
						// Check if a DBLEVEL-style setter exists (setXxx that converts dB to linear)
						String setterName = "set" + f.getName();
						try {
							getClass().getMethod(setterName, double.class);
							// Setter exists — this is a control panel variable.
							// 0.0 linear gain is not reachable via the slider,
							// so it must be a missing field from an older serialized version.
							f.setDouble(this, 1.0);
						} catch (NoSuchMethodException ignored) {
							// No setter — not a control panel variable, leave it alone
						}
					}
