...
class VolumeControlPanel_A {
			...
			public void run() {
				...
				gainField.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						try {
							int val = Integer.parseInt(gainField.getText().replaceAll("[^0-9.\\-]", "").split("\\.")[0]);
							val = Math.max(-48, Math.min(0, val));
							spbVol.setGain(val);
				...
				frame.getContentPane().add(gainField);
	...
	public void updateGainLabel() {
		gainField.setText("Gain " + String.format("%d dB", spbVol.getGain()));
	}
}