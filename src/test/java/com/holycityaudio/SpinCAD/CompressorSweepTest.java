...
/**
 * Compressor sweep test using the actual generated block.
 * Produces SVG with two rows of overlay plots:
 *   Top:    fixed threshold -25 dB, overlay ratios 2/4/6/8
 *   Bottom: fixed ratio 8:1, overlay thresholds -40/-30/-20/-10/0
 * Input and output axes both span 40 dB (-40 to 0).
 * No makeup gain applied.
 */
public class CompressorSweepTest {

    @TempDir
    File tempDir;

    // Input levels: -40 to 0 in 2 dB steps for smooth curves
    private static final int NUM_LEVELS = 21;
    private static double[] INPUT_DB;
    private static File[] inputWavs;

    // Chart 1: vary ratio at fixed threshold
    private static final double[] RATIOS = {2.0, 4.0, 6.0, 8.0};
    private static final double FIXED_THRESH = -25.0;

    // Chart 2: vary threshold at fixed ratio
    ...
    static void setup() throws IOException {
        ...
        for (int i = 0; i < NUM_LEVELS; i++) {
            INPUT_DB[i] = -40.0 + i * 2.0;  // -40, -38, ..., -2, 0
    ...
    void testSweepAndRenderSvg() throws Exception {
        // --- Chart 1: vary ratio at fixed threshold ---
        double[][] ratioResults = new double[RATIOS.length][NUM_LEVELS];
        for (int ri = 0; ri < RATIOS.length; ri++) {
            for (int li = 0; li < NUM_LEVELS; li++) {
                double rms = measureOutput(RATIOS[ri], FIXED_THRESH, 0.0625, 0.0,
                        inputWavs[li], String.format("ratio_r%d_l%d", ri, li));
                ratioResults[ri][li] = 20.0 * Math.log10(rms + 1e-15);
            }
        }
        ...
        for (int ti = 0; ti < THRESH_DB.length; ti++) {
            for (int li = 0; li < NUM_LEVELS; li++) {
                double rms = measureOutput(FIXED_RATIO, THRESH_DB[ti], 0.0625, 0.0,
        ...
        // Print tables
        System.out.println("\n=== Ratio sweep (threshold=" + FIXED_THRESH + " dB, no makeup) ===");
        for (int ri = 0; ri < RATIOS.length; ri++) {
            System.out.printf("Ratio %4.1f: ", RATIOS[ri]);
            for (int li = 0; li < NUM_LEVELS; li += 4) {
                System.out.printf("[%3.0f->%5.1f] ", INPUT_DB[li], ratioResults[ri][li]);
            }
            System.out.println();
        }

        System.out.println("\n=== Threshold sweep (ratio=" + FIXED_RATIO + ":1, no makeup) ===");
        for (int ti = 0; ti < THRESH_DB.length; ti++) {
        ...
        // Generate SVG
        File svgFile = new File("build/compressor_sweep.svg");
        svgFile.getParentFile().mkdirs();
        writeSvg(svgFile, ratioResults, threshResults);
        System.out.println("\nSVG written to: " + svgFile.getAbsolutePath());
    }

    private double measureOutput(double ratio, double threshDb, double knee,
            double makeupDb, File wavFile, String tag) throws Exception {
        ...
        comp.setinGain(1.0);
        comp.setknee(knee);
        comp.setavgTime(0.001);   // fast attack for steady-state measurement
    ...
    private void writeSvg(File file, double[][] ratioResults, double[][] threshResults)
            throws IOException {
        ...
        // --- Left plot: vary ratio ---
        int lx = 20, ly = 10;
        int lpx = lx + padL, lpy = ly + padT;

        drawPlot(pw, lpx, lpy, plotW, plotH, axMin, axMax, axMin, axMax,
                String.format("Vary Ratio (Threshold = %.0f dB, no makeup)", FIXED_THRESH),
                "Input (dBFS)", "Output (dBFS)");

        for (int ri = 0; ri < RATIOS.length; ri++) {
            drawCurve(pw, ratioResults[ri], lpx, lpy, plotW, plotH, axMin, axMax, COLORS_4[ri]);
        }
        ...
        drawPlot(pw, rpx, rpy, plotW, plotH, axMin, axMax, axMin, axMax,
                String.format("Vary Threshold (Ratio = %.0f:1, no makeup)", FIXED_RATIO),
                "Input (dBFS)", "Output (dBFS)");

        for (int ti = 0; ti < THRESH_DB.length; ti++) {
            drawCurve(pw, threshResults[ti], rpx, rpy, plotW, plotH, axMin, axMax, COLORS_5[ti]);
        }
    ...
    private void drawCurve(PrintWriter pw, double[] data, int px, int py,
            int plotW, int plotH, double axMin, double axMax, String color) {
        StringBuilder path = new StringBuilder();
        for (int i = 0; i < NUM_LEVELS; i++) {
            double inDb = INPUT_DB[i];
            double outDb = Math.max(axMin, Math.min(axMax, data[i]));
            double fx = (inDb - axMin) / (axMax - axMin);
            double fy = 1.0 - (outDb - axMin) / (axMax - axMin);
    ...
    private static File generateTestWav(double durationSeconds, double amplitude) throws IOException {
        ...
        File wavFile = File.createTempFile("comp_sweep_", ".wav");
