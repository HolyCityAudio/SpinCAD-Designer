...
/**
 * Peak compressor sweep test (Faust compression_gain_mono style).
 * Produces SVG with two overlay plots:
 *   Left:  fixed threshold -25 dB, overlay ratios 2/4/6/8
 *   Right: fixed ratio 8:1, overlay thresholds -40/-30/-20/-10/0
 * Input and output axes span -40 to 0 dBFS.
 */
public class PeakCompressorSweepTest {
    ...
    void testSweepAndRenderSvg() throws Exception {
        ...
        for (int ri = 0; ri < RATIOS.length; ri++) {
            for (int li = 0; li < NUM_LEVELS; li++) {
                double rms = measureOutput(RATIOS[ri], FIXED_THRESH, 0.0,
        ...
        for (int ti = 0; ti < THRESH_DB.length; ti++) {
            for (int li = 0; li < NUM_LEVELS; li++) {
                double rms = measureOutput(FIXED_RATIO, THRESH_DB[ti], 0.0,
        ...
        // Print tables
        System.out.println("\n=== Peak Compressor: Ratio sweep (threshold=" + FIXED_THRESH + " dB) ===");
        for (int ri = 0; ri < RATIOS.length; ri++) {
        ...
        System.out.println("\n=== Peak Compressor: Threshold sweep (ratio=" + FIXED_RATIO + ":1) ===");
        for (int ti = 0; ti < THRESH_DB.length; ti++) {
        ...
        // Generate SVG
        File svgFile = new File("build/peak_compressor_sweep.svg");
        svgFile.getParentFile().mkdirs();
        writeSvg(svgFile, ratioResults, threshResults);
        System.out.println("\nSVG written to: " + svgFile.getAbsolutePath());
    }

    private double measureOutput(double ratio, double threshDb, double makeupDb,
    ...
    private double[] computeYRange(double[][]... allResults) {
        ...
        // Round to nearest 5 dB, pad by 5 dB, enforce 40 dB span
    ...
    private void writeSvg(File file, double[][] ratioResults, double[][] threshResults)
        ...
        double[] yRangeL = computeYRange(ratioResults);
        ...
        drawPlot(pw, lpx, lpy, plotW, plotH, xMin, xMax, yRangeL[0], yRangeL[1],
                String.format("Peak Comp: Vary Ratio (Thresh = %.0f dB)", FIXED_THRESH),
                "Input (dBFS)", "Output (dBFS)");

        for (int ri = 0; ri < RATIOS.length; ri++) {
            drawCurve(pw, ratioResults[ri], lpx, lpy, plotW, plotH, xMin, xMax, yRangeL[0], yRangeL[1], COLORS_4[ri]);
        }
        ...
        drawPlot(pw, rpx, rpy, plotW, plotH, xMin, xMax, yRangeR[0], yRangeR[1],
                String.format("Peak Comp: Vary Threshold (Ratio = %.0f:1)", FIXED_RATIO),
                "Input (dBFS)", "Output (dBFS)");
    ...
    private static File generateTestWav(double durationSeconds, double amplitude) throws IOException {
        ...
        File wavFile = File.createTempFile("peak_comp_sweep_", ".wav");
