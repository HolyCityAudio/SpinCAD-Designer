...
public class Pitch_shift_testTest {
	public Pitch_shift_testTest(SpinCADFrame f) {
		System.out.println("Start of Pitch_shift_testCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		Pitch_shift_testCADBlock m = new Pitch_shift_testCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("Pitch_shift_testCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Pitch Out");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Pitch Control");
	p2.setConnection(pot0, p1);
}
f.getModel().sortAlignGen();
System.out.println("Pitch_shift_testCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of Pitch_shift_testCADBlock unit test" + "\n");
	}
}
