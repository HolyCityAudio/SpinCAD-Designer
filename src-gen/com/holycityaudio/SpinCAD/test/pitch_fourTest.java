...
public class pitch_fourTest {
	public pitch_fourTest(SpinCADFrame f) {
		System.out.println("Start of pitch_fourCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		pitch_fourCADBlock m = new pitch_fourCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("pitch_fourCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Pitch_Out");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Pitch_Select");
	p2.setConnection(pot0, p1);
}
f.getModel().sortAlignGen();
System.out.println("pitch_fourCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of pitch_fourCADBlock unit test" + "\n");
	}
}
