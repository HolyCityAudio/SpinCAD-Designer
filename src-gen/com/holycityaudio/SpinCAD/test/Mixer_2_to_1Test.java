...
public class Mixer_2_to_1Test {
	public Mixer_2_to_1Test(SpinCADFrame f) {
		System.out.println("Start of Mixer_2_to_1CADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		Mixer_2_to_1CADBlock m = new Mixer_2_to_1CADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("Mixer_2_to_1CADBlock test passed with no connections!");
f.getModel().newModel();
...
f.dropBlock(p, o);	
{
	...
	SpinCADPin p2 = m.getPin("Input 1");
	p2.setConnection(i, p1);
}
{
	SpinCADPin p1 = i.getPin("Output 2");
	SpinCADPin p2 = m.getPin("Input 2");
	p2.setConnection(i, p1);
}
{
...
}
{
	...
	SpinCADPin p2 = m.getPin("Level 1");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Level 2");
	p2.setConnection(pot1, p1);
}
f.getModel().sortAlignGen();
System.out.println("Mixer_2_to_1CADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of Mixer_2_to_1CADBlock unit test" + "\n");
	}
}
