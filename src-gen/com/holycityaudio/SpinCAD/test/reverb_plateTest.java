...
public class reverb_plateTest {
	public reverb_plateTest(SpinCADFrame f) {
		System.out.println("Start of reverb_plateCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		reverb_plateCADBlock m = new reverb_plateCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("reverb_plateCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("reverb_plateCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of reverb_plateCADBlock unit test" + "\n");
	}
}
