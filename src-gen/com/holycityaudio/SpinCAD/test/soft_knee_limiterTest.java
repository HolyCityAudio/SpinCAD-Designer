...
public class soft_knee_limiterTest {
	public soft_knee_limiterTest(SpinCADFrame f) {
		System.out.println("Start of soft_knee_limiterCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		soft_knee_limiterCADBlock m = new soft_knee_limiterCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("soft_knee_limiterCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("soft_knee_limiterCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of soft_knee_limiterCADBlock unit test" + "\n");
	}
}
