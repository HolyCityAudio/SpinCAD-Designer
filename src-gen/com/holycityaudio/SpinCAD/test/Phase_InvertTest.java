...
public class Phase_InvertTest {
	public Phase_InvertTest(SpinCADFrame f) {
		System.out.println("Start of Phase_InvertCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		Phase_InvertCADBlock m = new Phase_InvertCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("Phase_InvertCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("Phase_InvertCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of Phase_InvertCADBlock unit test" + "\n");
	}
}
