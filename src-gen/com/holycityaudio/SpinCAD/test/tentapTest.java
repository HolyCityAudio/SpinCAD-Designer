...
public class tentapTest {
	public tentapTest(SpinCADFrame f) {
		System.out.println("Start of tentapCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		tentapCADBlock m = new tentapCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("tentapCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Mix_2_Out");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("tentapCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of tentapCADBlock unit test" + "\n");
	}
}
