...
public class NotchTest {
	public NotchTest(SpinCADFrame f) {
		System.out.println("Start of NotchCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		NotchCADBlock m = new NotchCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("NotchCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Output_Notch");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Output_Bandpass");
	p2.setConnection(o, p1);
}
{
	...
	SpinCADPin p2 = m.getPin("Freqeuncy");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Resonance");
	p2.setConnection(pot1, p1);
}
f.getModel().sortAlignGen();
System.out.println("NotchCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of NotchCADBlock unit test" + "\n");
	}
}
