...
public class noise_amzTest {
	public noise_amzTest(SpinCADFrame f) {
		System.out.println("Start of noise_amzCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		noise_amzCADBlock m = new noise_amzCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("noise_amzCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("noise_amzCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of noise_amzCADBlock unit test" + "\n");
	}
}
