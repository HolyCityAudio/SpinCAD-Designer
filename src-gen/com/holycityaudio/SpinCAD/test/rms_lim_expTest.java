...
public class rms_lim_expTest {
	public rms_lim_expTest(SpinCADFrame f) {
		System.out.println("Start of rms_lim_expCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		rms_lim_expCADBlock m = new rms_lim_expCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("rms_lim_expCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("rms_lim_expCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of rms_lim_expCADBlock unit test" + "\n");
	}
}
