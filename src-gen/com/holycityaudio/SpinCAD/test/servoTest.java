...
public class servoTest {
	public servoTest(SpinCADFrame f) {
		System.out.println("Start of servoCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		servoCADBlock m = new servoCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("servoCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Tap Output");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("servoCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of servoCADBlock unit test" + "\n");
	}
}
