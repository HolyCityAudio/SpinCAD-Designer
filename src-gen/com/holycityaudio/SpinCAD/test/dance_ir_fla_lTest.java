...
public class dance_ir_fla_lTest {
	public dance_ir_fla_lTest(SpinCADFrame f) {
		System.out.println("Start of dance_ir_fla_lCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		dance_ir_fla_lCADBlock m = new dance_ir_fla_lCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("dance_ir_fla_lCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.dropBlock(p, o);	
{
	...
	SpinCADPin p2 = m.getPin("Input_L");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Low_Pass");
	p2.setConnection(pot2, p1);
}
f.getModel().sortAlignGen();
System.out.println("dance_ir_fla_lCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of dance_ir_fla_lCADBlock unit test" + "\n");
	}
}
