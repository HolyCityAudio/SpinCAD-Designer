...
public class aliaserTest {
	public aliaserTest(SpinCADFrame f) {
		System.out.println("Start of aliaserCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		aliaserCADBlock m = new aliaserCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("aliaserCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.dropBlock(p, o);	
{
	...
	SpinCADPin p2 = m.getPin("Audio_Input");
...
f.getModel().sortAlignGen();
System.out.println("aliaserCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of aliaserCADBlock unit test" + "\n");
	}
}
