 ...
 * very_tight_plate_verbTest.java
...
public class very_tight_plate_verbTest {
	public very_tight_plate_verbTest(SpinCADFrame f) {
		System.out.println("Start of very_tight_plate_verbCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		very_tight_plate_verbCADBlock m = new very_tight_plate_verbCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("very_tight_plate_verbCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Output 1");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Output 2");
	p2.setConnection(o, p1);
}
{
...
}
{
	...
	SpinCADPin p2 = m.getPin("LF_Loss");
...
}
{
	...
	SpinCADPin p2 = m.getPin("HF_Loss");
	p2.setConnection(pot2, p1);
}
f.getModel().sortAlignGen();
System.out.println("very_tight_plate_verbCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of very_tight_plate_verbCADBlock unit test" + "\n");
	}
}
