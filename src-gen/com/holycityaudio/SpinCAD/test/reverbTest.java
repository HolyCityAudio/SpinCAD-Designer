...
public class reverbTest {
	public reverbTest(SpinCADFrame f) {
		System.out.println("Start of reverbCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		reverbCADBlock m = new reverbCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("reverbCADBlock test passed with no connections!");
f.getModel().newModel();
...
// end of test code
	System.out.println("End of reverbCADBlock unit test" + "\n");
	}
}
