...
public class sixtapTest {
	public sixtapTest(SpinCADFrame f) {
		System.out.println("Start of sixtapCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		sixtapCADBlock m = new sixtapCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("sixtapCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Feedback In");
	p2.setConnection(i, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 1");
	SpinCADPin p2 = m.getPin("Mix L Out");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Mix R Out");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("sixtapCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of sixtapCADBlock unit test" + "\n");
	}
}
