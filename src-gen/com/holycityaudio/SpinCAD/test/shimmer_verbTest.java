...
public class shimmer_verbTest {
	public shimmer_verbTest(SpinCADFrame f) {
		System.out.println("Start of shimmer_verbCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		shimmer_verbCADBlock m = new shimmer_verbCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("shimmer_verbCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Damping");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Decay");
	p2.setConnection(pot2, p1);
}
f.getModel().sortAlignGen();
System.out.println("shimmer_verbCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of shimmer_verbCADBlock unit test" + "\n");
	}
}
