...
public class VolumeTest {
	public VolumeTest(SpinCADFrame f) {
		System.out.println("Start of VolumeCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		VolumeCADBlock m = new VolumeCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("VolumeCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Volume");
	p2.setConnection(pot0, p1);
}
f.getModel().sortAlignGen();
System.out.println("VolumeCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of VolumeCADBlock unit test" + "\n");
	}
}
