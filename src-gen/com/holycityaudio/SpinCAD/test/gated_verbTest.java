...
public class gated_verbTest {
	public gated_verbTest(SpinCADFrame f) {
		System.out.println("Start of gated_verbCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		gated_verbCADBlock m = new gated_verbCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("gated_verbCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Gate_Time");
...
f.getModel().sortAlignGen();
System.out.println("gated_verbCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of gated_verbCADBlock unit test" + "\n");
	}
}
