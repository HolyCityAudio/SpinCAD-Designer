...
public class ga_demo_vibratoTest {
	public ga_demo_vibratoTest(SpinCADFrame f) {
		System.out.println("Start of ga_demo_vibratoCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		ga_demo_vibratoCADBlock m = new ga_demo_vibratoCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("ga_demo_vibratoCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Vibrato_Rate");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Vibrato_Width");
	p2.setConnection(pot2, p1);
}
f.getModel().sortAlignGen();
System.out.println("ga_demo_vibratoCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of ga_demo_vibratoCADBlock unit test" + "\n");
	}
}
