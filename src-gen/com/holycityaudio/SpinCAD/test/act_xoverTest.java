...
public class act_xoverTest {
	public act_xoverTest(SpinCADFrame f) {
		System.out.println("Start of act_xoverCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		act_xoverCADBlock m = new act_xoverCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("act_xoverCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Output_L");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Output_R");
	p2.setConnection(o, p1);
}
f.getModel().sortAlignGen();
System.out.println("act_xoverCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of act_xoverCADBlock unit test" + "\n");
	}
}
