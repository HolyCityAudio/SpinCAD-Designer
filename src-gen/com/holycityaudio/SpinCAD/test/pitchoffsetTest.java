...
public class pitchoffsetTest {
	public pitchoffsetTest(SpinCADFrame f) {
		System.out.println("Start of pitchoffsetCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		pitchoffsetCADBlock m = new pitchoffsetCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("pitchoffsetCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Pitch_Offset");
	p2.setConnection(pot0, p1);
}
f.getModel().sortAlignGen();
System.out.println("pitchoffsetCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of pitchoffsetCADBlock unit test" + "\n");
	}
}
