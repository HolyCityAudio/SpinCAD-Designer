...
public class rom_rev1Test {
	public rom_rev1Test(SpinCADFrame f) {
		System.out.println("Start of rom_rev1CADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		rom_rev1CADBlock m = new rom_rev1CADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("rom_rev1CADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Low_Freq");
...
}
{
	...
	SpinCADPin p2 = m.getPin("High_Freq");
	p2.setConnection(pot2, p1);
}
f.getModel().sortAlignGen();
System.out.println("rom_rev1CADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of rom_rev1CADBlock unit test" + "\n");
	}
}
