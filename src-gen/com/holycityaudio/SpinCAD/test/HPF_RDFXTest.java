...
public class HPF_RDFXTest {
	public HPF_RDFXTest(SpinCADFrame f) {
		System.out.println("Start of HPF_RDFXCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		HPF_RDFXCADBlock m = new HPF_RDFXCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("HPF_RDFXCADBlock test passed with no connections!");
f.getModel().newModel();
...
f.getModel().sortAlignGen();
System.out.println("HPF_RDFXCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of HPF_RDFXCADBlock unit test" + "\n");
	}
}
