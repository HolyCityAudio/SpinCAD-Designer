...
public class TripleTapTest {
	public TripleTapTest(SpinCADFrame f) {
		System.out.println("Start of TripleTapCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		TripleTapCADBlock m = new TripleTapCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("TripleTapCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Tap2_Out");
	p2.setConnection(o, p1);
}
{
...
}
{
	...
	SpinCADPin p2 = m.getPin("Delay_Time_2");
...
// end of test code
	System.out.println("End of TripleTapCADBlock unit test" + "\n");
	}
}
