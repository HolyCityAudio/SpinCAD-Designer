...
public class crossfadeTest {
	public crossfadeTest(SpinCADFrame f) {
		System.out.println("Start of crossfadeCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		crossfadeCADBlock m = new crossfadeCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("crossfadeCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Audio Output");
...
}
{
	...
	SpinCADPin p2 = m.getPin("Fade");
	p2.setConnection(pot0, p1);
}
f.getModel().sortAlignGen();
System.out.println("crossfadeCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of crossfadeCADBlock unit test" + "\n");
	}
}
