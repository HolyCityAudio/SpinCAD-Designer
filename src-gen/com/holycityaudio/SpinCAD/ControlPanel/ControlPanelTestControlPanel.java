...
public class ControlPanelTestControlPanel extends spinCADControlPanel {
	...
	FineControlSlider pitchCoeff1Slider;
	JTextField  pitchCoeff1Field;
	FineControlSlider pitchCoeff2Slider;
	JTextField  pitchCoeff2Field;
...
public ControlPanelTestControlPanel(ControlPanelTestCADBlock genericCADBlock) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame = new JDialog(SpinCADFrame.getInstance(), "Control Panel Test");
				frame.setLayout(new BoxLayout(frame.getContentPane(), BoxLayout.Y_AXIS));

			//
			// these functions translate between slider values, which have to be integers, to whatever in program value you wish.
			//
					pitchCoeffSlider = new FineControlSlider(JSlider.HORIZONTAL, (int)(-16384 * 1.0),(int) (32767 * 1.0), (int) (gCB.getpitchCoeff() * 1.0));
			...
			// these functions translate between slider values, which have to be integers, to whatever in program value you wish.
			//
					pitchCoeff1Slider = new FineControlSlider(JSlider.HORIZONTAL, (int)(-16384 * 1.0),(int) (32767 * 1.0), (int) (gCB.getpitchCoeff1() * 1.0));
					//---------------------------------------------
					...
					// ---------------------------------------------						
						...
						pitchCoeff1Field.setBorder(pitchCoeff1Border1);
						pitchCoeff1Field.addActionListener(new java.awt.event.ActionListener() {
							@Override
							public void actionPerformed(java.awt.event.ActionEvent e) {
								try {
									double val = Double.parseDouble(pitchCoeff1Field.getText().replaceAll("[^0-9.\\-]", ""));
						double samples = val * ElmProgram.getSamplerate() / 1000.0;
						int sliderVal = (int) Math.round(samples * 1.0);
						sliderVal = Math.max(pitchCoeff1Slider.getMinimum(), Math.min(pitchCoeff1Slider.getMaximum(), sliderVal));
						pitchCoeff1Slider.setValue(sliderVal);
						gCB.setpitchCoeff1((double) sliderVal / 1.0);
						...
						pitchCoeff1innerPanel.add(pitchCoeff1Field);
						pitchCoeff1innerPanel.add(Box.createRigidArea(new Dimension(5,4)));
			...
			//
					...
					pitchCoeff2Slider = new FineControlSlider(JSlider.HORIZONTAL, (int)(-16384 * 1.0),(int) (32767 * 1.0), (int) ((gCB.getpitchCoeff2()) * 1.0));
						...
						pitchCoeff2Field.setHorizontalAlignment(JTextField.CENTER);
						Border pitchCoeff2Border1 = BorderFactory.createBevelBorder(BevelBorder.LOWERED);
						pitchCoeff2Field.setBorder(pitchCoeff2Border1);
						pitchCoeff2Field.addActionListener(new java.awt.event.ActionListener() {
							@Override
							public void actionPerformed(java.awt.event.ActionEvent e) {
								try {
									double val = Double.parseDouble(pitchCoeff2Field.getText().replaceAll("[^0-9.\\-]", ""));
						int sliderVal = (int) Math.round(val * 1.0);
						sliderVal = Math.max(pitchCoeff2Slider.getMinimum(), Math.min(pitchCoeff2Slider.getMaximum(), sliderVal));
						pitchCoeff2Slider.setValue(sliderVal);
						gCB.setpitchCoeff2((double) sliderVal / 1.0);
						...
						pitchCoeff2innerPanel.add(pitchCoeff2Field);
						pitchCoeff2innerPanel.add(Box.createRigidArea(new Dimension(5,4)));
