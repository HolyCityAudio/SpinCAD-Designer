...
_A practical guide to taming the 0–1 world of the FV-1_
...
![Power block shaping: normal vs inverted](.gitbook/assets/control_signal_power_demo.png)

The key insight is that the Power block reshapes _where the signal spends its time_ without changing the endpoints. Normal power makes it linger at the low end; inverted power makes it linger at the high end. The Scale/Offset block then maps that shaped signal into whatever output range your destination parameter needs.
